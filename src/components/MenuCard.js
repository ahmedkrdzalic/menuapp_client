...
function MenuCard(props) {
    ...
    if(props.props.add){
        ...
        return(
            <button onClick={() => props.props.setShowModal(true)} className="cursor-pointer text-teal-500 text-4xl basis-72 aspect-video shrink-0 rounded overflow-hidden hover:bg-gray-300 border-2 border-teal-500 justify-center text-center bg-gray-100 flex flex-col flex-wrap content-center max-h-64">
                <span className=''>+</span>
            </button>
        )
    }
  ...
  return (
    ...
    className="text-slate-500 basis-72 aspect-video shrink-0 rounded overflow-hidden border-2 border-teal-500 justify-center text-center bg-gray-100 flex flex-col flex-wrap content-center max-h-64 relative ">
        <div className="px-6 py-4 break-words">
            <p className="font-bold text-xl mb-2">{props.props.menu.title}</p>
        </div>
        <div className='p-5'>
        ...
        <div className="absolute bottom-0 right-0 flex flex-row flex-nowrap">
            <button 
            onClick={() => {navigate(`/editmenu/${props.props.menu.id}`)}} 
            ...
            <button onClick={(e) => {onClickDeleteMenu(props.props.menu.id)}} className="inline-block bg-gray-200 hover:bg-gray-300 rounded px-2 py-1 text-sm font-semibold mr-2 mb-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
